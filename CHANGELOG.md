...
### Changed
- 25 null checks guarding a single assignment use C# 14's null-conditional assignment instead, so `if (Polygon != null) { Polygon.Fill = ...; }` is `Polygon?.Fill = ...;`. The right side is not evaluated when the target is null, which is what the `if` did, and the six that subscribe or unsubscribe an event behave the same way through `?.`. Two of them were the assignment bodies that the brace convention asks to wrap, and dropping the control flow removes the question. The two remaining braceless assignment bodies in the libraries, in `TaskInstance` and `TabZipFile`, are wrapped now. The two inside commented out methods, in `Serializer` and `TabModel`, are left alone
- JSON deserialization rejects duplicate properties rather than silently keeping the last value, through .NET 10's `JsonSerializerOptions.AllowDuplicateProperties`. `{"a":1,"a":2}` used to resolve to 2, which is the shape a parser differential takes: a check reads one value and the code acting on it reads the other. It is set on `JsonConverters.CreatePublicOptions()` and `CreatePrivateOptions()`, which cover bookmark links, the JSON serializer files, and the key value stores, on the theme options that parse an imported theme, and on `HttpMemoryCache`, which parses fetched responses. Well formed JSON is unaffected. The call sites that deserialize with no options at all — the `JsonHeader` and `StorageHeader` reads and the browser local storage reads — still accept duplicates, since giving each one options is a larger change
- 35 properties across 26 files use the C# 14 `field` keyword rather than declaring a backing field of their own, which removes the separate declaration and the chance of the two drifting apart. Behavior is unchanged: a property initializer assigns the backing field directly, exactly as the field initializer it replaces did, so the validation in a setter is bypassed at construction the same way it always was. Eleven properties keep their explicit field, either because something outside the property reads or writes it, because two properties share one field as `TabModel.ItemList` and `TabModel.ItemLists` do, or because the field type differs from the property type — `TaskInstance.TaskCount` is an `int` over an `int?` that distinguishes unset from zero, which `field` cannot express since it takes the property type exactly
- `ObjectExtensions.WithIndex()` is obsolete, superseded by `Enumerable.Index()` which .NET 9 added, and forwards to it. Its own doc comment had named it as the replacement to adopt. The four call sites use `Index()` directly now, which means `x.Index` and `x.Item` rather than `x.index` and `x.item` where the tuple is accessed by name rather than deconstructed. The obsolete forwarder is kept for a release rather than removed outright, since it is public API
