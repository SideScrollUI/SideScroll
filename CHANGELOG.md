...
- Fixed reordering columns throwing for a type that redeclares a property. `TabDataColumns.GetOrderedPropertyColumns()` keyed the visible properties by name, and reflection returns both declarations for a property a subclass redeclares with a different type, so building that lookup threw `ArgumentException` for the duplicate. It only ran once columns had been reordered, so a grid rendered fine until the user dragged a column and then threw from then on, with the saved order keeping it broken. The last declaration for a repeated name wins now, which is the derived one, matching what `ListProperty.Create()` already does
