...
### Fixed
- Fixed a member a subclass redeclares with `new` binding the hidden base declaration instead. Reflection returns both when it can't resolve which one the compiler means, always for a field and for a property whose type changed, and `ReflectionCache` sorted them by `MetadataToken` and let the last win. That follows declaration order, so a subclass declared above the type it derives from bound the base member and showed its value in every grid, form, and inline expansion. `MemberExtensions.RemoveHidden()` picks the declaration that hides the others now, and `GetMostDerived()` moved there from `TabTextBox` since both need it
- Fixed `[Hidden]` on a member a subclass redeclares with `new` showing the base member's value instead of hiding the row, which a redeclaration without a public getter did too. The redeclaration was dropped by the visibility filters before the two declarations were collapsed, leaving the one it hides as the only member of that name
