...
- `SerializerKeyValueStore.LoadHeader()` logs a header it can't read and treats it as absent, the way it already treats unreadable data. The browser repository listings call it for every item in the group, so one damaged header entry used to fail the listing of all of them, where the file-based repository skips the one item
- `HttpUtils.HttpGetProgress.Percent` reports 100 for a `Content-Length: 0` body rather than NaN
- `CompressionUtils.Decompress()` compares each zip entry's declared size against the remaining allowance before adding it to the total, so ZIP64 lengths that summed past `long.MaxValue` and wrapped negative can no longer pass the extracted-size limit. A negative declared length is rejected the same way
- `ChartView.AddDimensions()` accepts an array or any other `IList` without a parameterless constructor as the source. Each dimension's list is now a `List<T>` of the element type rather than an instance of the source list's own type, which also means a collection that seeds itself from its constructor no longer starts every dimension with those items
- A `DateTime?` X-axis property is a time series like a `DateTime` one. `ListSeries.TimeRangeValues` compared the property type to `DateTime` exactly, so a nullable axis had no time window, couldn't be grouped by period, and was drawn as non-temporal. Items whose X value is null are skipped, the way a null Y value already was
- `TimeRangePeriod.Tags` no longer throws `ArgumentNullException` for a tag with no name; unnamed tags are grouped together
- `DataItemCollection.Add()` checks for an existing key before adding, so a duplicate throws without first putting the item in the list while the lookup still held the original
- `TimeZoneView.ConvertTimeToUtc()` returns a value whose `Kind` is already `Utc` unchanged and converts a `Local` one from the machine's zone, whatever zone the view is for. The Local and named-zone branches relabelled either as wall-clock time in the view's zone and shifted it by the offset, so `Convert()` followed by `ConvertTimeToUtc()` moved the value by the zone's offset. An `Unspecified` value is still read as wall-clock time in the view's zone
- Lazy loading works for a generic type with a virtual property. `LazyClass` built its dynamic assembly name by parsing `"Lazy." + FullName`, and a generic type's `FullName` carries its arguments in brackets that `AssemblyName` rejected with `FileLoadException`
- A lazy property reference whose object index or type index is out of range loads as null, the way the eager path already treated one. `TypeRepo.LoadFullObject()` indexed the loaded objects unchecked, so a truncated or edited file threw `IndexOutOfRangeException` from inside the property getter after `Load()` had reported success
- `DataPageView` and `TaskCreator` no longer fall back to a bare `SynchronizationContext` when there is none to capture, which posted every later change to the thread pool; with no context they raise changes on the calling thread, as `ItemCollectionUI` and `BookmarkNavigator` already do. `TaskInstance.SetFinished()` runs `OnFinished()` in place when the creator has no context
- `RoundToSignificantFigures(decimal)` returns the input when the shift needs more than the 28 decimal places a `decimal` holds, or rounding would carry past its range, rather than throwing `OverflowException`. The double overload already returned the input in the same cases
- A task created from a lambda or a local function is labelled by the enclosing method or the local function's own name rather than the compiler's mangled `<Main>b__0_5`, through the new `MethodInfo.GetSourceName()` extension. `TaskDelegate`, `TaskDelegateAsync`, and `Call.RunAsync()` use it
- `ProcessUtils.OpenFolder()` opens a folder path that ends in a separator. Windows argument parsing reads a backslash before the closing quote as an escape, so `"C:\Users\Public\"` reached explorer as `C:\Users\Public"` and it opened its default folder. The separator is trimmed, and a root's is doubled
- `FileUtils.IsFileOpen()` returns false for a directory or a file the process can't read rather than throwing `UnauthorizedAccessException`
- `DateTimeValueConverter` formats the time text with the invariant culture, so it always carries the colon that `TryParseTimeSpan()` requires. Under fi-FI and other cultures whose time separator is a period, the picker displayed `14.30.45` and then rejected its own text on edit
- A bookmark link whose `$type` or `Type` value names an assembly that can't be parsed (`"Foo, Bar, Version=abc"`) imports with that member null rather than failing outright. `ObjectJsonConverter` and `TypeJsonConverter` passed `throwOnError: false` to `Type.GetType()`, which only covers a type that isn't found; a malformed assembly name threw `FileLoadException` from every overload
- `TabFormattedComboBox` accepts a choice list containing null, which is the natural way to offer "none" and which its selection code already expected. `FormattedItem.Create()` called `ToString()` on each choice to deduplicate, so a null threw `NullReferenceException` while the form or toolbar was being built
- The live-chart tooltip formats a `DateTime?` X axis as a time. It compared the property type to `DateTime` exactly, the same check CHT-045 fixed in `ListSeries`, so a nullable axis plotted correctly but showed its raw ticks on hover
- The Atlas serializer saves `nint` and `nuint` members as 64-bit values, the way it stores a `float` as a double. `TypeRepoPrimitive` claimed every primitive type but handled twelve, so a class with a native-sized integer failed to save with "Unhandled primitive type". A value loaded on a 32-bit platform that doesn't fit throws `OverflowException`, which the loader reports for that object
- `DataRepoIndex.MaxItems` rejects zero along with negatives. `Save()` adds to the index before writing the data, so a limit of zero pruned every item as it was added and then wrote data that no indexed listing would show; `null` is still unlimited
- `ProcessUtils.OpenBrowser()` and `OpenFolder()` on Linux and macOS pass the URL or path to `xdg-open` and `open` as a single argument through `ArgumentList`. They went through the arguments string, which is tokenized on unquoted whitespace and quotes before the child sees it, so a path or URL containing a space opened only the part before it
...
- JSON deserialization rejects duplicate properties rather than silently keeping the last value, through .NET 10's `JsonSerializerOptions.AllowDuplicateProperties`. `{"a":1,"a":2}` used to resolve to 2, which is the shape a parser differential takes: a check reads one value and the code acting on it reads the other. It is set on `JsonConverters.CreatePublicOptions()` and `CreatePrivateOptions()`, which cover bookmark links, the JSON serializer files, and the key value stores, on the theme options that parse an imported theme, and on `HttpMemoryCache`, which parses fetched responses. Well formed JSON is unaffected. The call sites that deserialize with no options at all, the `JsonHeader` and `StorageHeader` reads and the browser local storage reads, still accept duplicates

### Deprecated
- `ObjectExtensions.WithIndex()` is obsolete in favor of `Enumerable.Index()`, which .NET 9 added, and forwards to it. `Index()` names its tuple elements `Index` and `Item` rather than `index` and `item`, so code reading the tuple by name needs the capitalized names when it switches, while code deconstructing it is unaffected
...
- Added `MemberNameMerger<T>`, which collapses members reflection returns more than once under the same name, keeping the most derived declaration at the position of the one it hides. `ListProperty.Create()`, `ListField.Create()`, `ListMethod.Create()`, `ListMember.Create()`, and `TabDataColumns` each implemented that rule separately, and two of them differed
- Added `DataViewCollection.DeleteConfirmation`, a `ConfirmationFlyoutConfig` prompting before a row's delete button removes it, alongside `EnableDeleting` which still deletes immediately when it's left null. It's carried on `IDeletableList` and reaches the grid through the new `DataGridButtonColumn.Confirmation`, so a `[ButtonColumn]` method can prompt the same way. The flyout is built on the first click and shared by every row, rather than one per cell for rows that may never be clicked, and the row is captured when clicked since the cell can be recycled for another row while the prompt is open. The theme list confirms its deletes now
- Added themed disabled button colors, `Button[Background|Foreground|BorderBrush]Disabled` and a `ButtonWarning` set beside them, in both theme variants and the Button theme section. Nothing themed the disabled state before, so it fell through to Avalonia's translucent grey overlay, which composites *lighter* than the panel it sits on — a disabled button drew 1.6x the contrast of an enabled one in the dark variant and 1.3x in the light, making the controls you can't press the loudest thing in a panel. The overlay is also neutral, so a disabled warning button was indistinguishable from a disabled default one. The new colors recede toward the background instead, below their enabled counterparts, and keep each accent's hue. `TabTextButton.UseWarningAccent()` overrides the disabled keys alongside the ones it already sets, since it themes a button through its local resources
- Added `TaskCreator.IsEnabledBinding`, which enables or disables an action's button from a bound property the way `ToolButton.IsEnabledBinding` already does for toolbar buttons. An action button could only be disabled by reaching for a raw Avalonia control before this, so the state was unreachable from the tabs layer that defines the actions. `PropertyBinding` moved from `SideScroll.Tabs.Toolbar` to `SideScroll` so `TaskCreator`, which sits below `SideScroll.Tabs`, can reach it — it resolves without a `using` from anything already under the `SideScroll` namespace, and anything outside it needs `using SideScroll;`. `TabSampleActions` demonstrates it with a checkbox disabling a default and a warning accented button
- Added "Copy - Name" and "Copy - Row" to the chart legend's context menu, which copy the series name and its name with its total, for a single series. The menu only copied the whole legend, all of it or the selected series, so pulling one name out of a chart of many meant copying every row and editing the rest away. The legend panel owns one menu for every item in it, so the item is resolved from where the menu was opened, and both entries are disabled when it's opened over the empty space beside the items. The name is copied without the rank prefix the label shows

### Fixed
- Fixed a chart legend series name too long to fit pushing its total out of the total column and past the edge of the legend. A legend item is a grid of `Auto, *, Auto`, and the name's `TextBlock` had no trimming, so it reported the width the whole name wanted rather than the width the star column gave it. The grid widened that column to the name's desired width, which left the total column starting further right than the rows whose names fit, and ending outside the legend, where it was clipped — the longer the name, the further right its total sat and the more of it was cut off. The name is trimmed with a character ellipsis now, which also clamps what it asks for to what's available, so every total lines up on the same edge and the name is cut off with a `…` rather than mid-character. The legend's column header was inset 4px less than the totals below it and is aligned with them now
- Fixed a warning accented button keeping the previous theme's colors until its tab was reopened. `TabTextButton.UseWarningAccent()` redirects the button's state colors by adding brushes to its local resources, and it read those brushes out of the theme once, when the button was constructed. A resource dictionary entry holds whatever object it was given rather than re-resolving the way the `DynamicResource` lookups in the control theme do, so editing any warning color in the theme settings left every existing warning button showing the old one, while the default accent — which has no local override and resolves through the control theme — updated immediately. The overrides are re-applied on `ActualThemeVariantChanged` now, the way `ToolbarTextBox` and `TabImageButton` already refresh theirs, and they're assigned rather than added so re-applying replaces them instead of throwing. `TabColorPicker` overrode its dropdown glyph and disabled text colors the same way and is refreshed alongside it
- Fixed a logarithmic chart drawing identical values along the bottom of its axis. `TabLiveChart.UpdateYAxis()` scaled the log limits by 0.85 and 1.15 instead of adding a margin to them, and scaling shrinks that margin to nothing as the log value approaches zero, so a series whose values are all 1 was given no separation at all and was drawn on the axis line itself. The lower limit was clamped to the log of 1 as well, so a series with fractional values was cut off below it. The margin is added in log space now, a full order of magnitude when every value is identical, and the limits follow the data rather than how far it happens to sit from where the log crosses zero
- Fixed a type that redeclares a property showing two columns of the same name, and throwing once those columns were reordered. Reflection returns both declarations for a property a subclass redeclares with a different type, and `TabDataColumns.GetVisibleProperties()` passed both through, so the grid drew a duplicate column bound to the declaration the other hides. `GetOrderedPropertyColumns()` keyed by name on top of that and threw `ArgumentException` for the duplicate, which only ran once a column had been dragged, so a grid rendered until it was reordered and then threw from then on, with the saved order keeping it broken. The declarations are merged where they're cached now, so the filter and the column count heuristic see one as well, and the kept declaration takes the position of the one it hides rather than its own, which is where `ListProperty.Create()` has always put it
- Fixed `ListMember.Create()` showing two rows for a field a subclass redeclares. Only the properties and methods were merged by name, and reflection returns both declarations for any `new` field, whether or not it changes the type, so one showed twice with the first row bound to the declaration the other hides. Fields are merged the same way `ListField.Create()` already merges its own now
...
- Fixed `LogUtils.Save()` writing outside the directory it was given. The caller supplied prefix names the log file and was concatenated straight into it, with nothing rejecting a separator, a parent segment, or a root — `../other/name` wrote outside the directory, and a rooted or drive qualified prefix made `Path.Combine()` discard it entirely. A prefix that isn't a plain file name throws an `ArgumentException` now
- Fixed the toolbar model's bindings being released by the control rendering them. A `ToolToggleButton`'s `ListProperty` subscribes to the object it's bound to, and `ToolbarToggleButton.Dispose()` disposed it, so a model rendered in two toolbars or rendered again after a reload was left bound to a property that no longer observed anything. `TabToolbar` owns and disposes those bindings now, the control releases only its own subscription, and `TabModel` disposes the toolbars among its objects so a closing tab reaches them. `TabControlToolbar.LoadToolbar()` disposes the controls it replaces, which is now safe
- Fixed `TabDataColumns.GetVisibleProperties()` handing out the list it cached. Every caller asking about a type got the same `List<PropertyInfo>`, so one of them sorting, clearing, or appending through it changed the columns for every later grid and schema export of that type, for the rest of the process. It returns an `IReadOnlyList<PropertyInfo>` over a cached `ReadOnlyCollection` now, which refuses the mutation rather than only discouraging it
- Fixed `TabDataColumns.GetMethodColumns()` returning button columns the grid can't invoke. It accepted every method carrying `[ButtonColumn]`, including ones taking parameters and generic definitions, and `DataGridButtonColumn` invokes with no arguments, so pressing the button threw. Those are left out of the columns now
- Fixed one toolbar property stopping the rest of the toolbar from rendering. `TabControlToolbar.LoadToolbar()` calls every visible property's getter before it knows whether the value is a control, so a computed property that threw ended the loop and lost every control after it. A throwing getter skips that one property now
- Fixed a `TimeRangeValue` without a tag list aborting the aggregation of its whole series. `Tags` is public, settable, and deserialized, so JSON or application code can leave it null, and `Description`, `Periods()`, and the result projection all dereference it, so one such value threw `ArgumentNullException` for every value around it. Reading it returns an empty list now, whichever way it was left unset. The backing field is private, which the serializer doesn't collect, so what's persisted is unchanged
- Fixed `TabControlToolbar.LoadToolbar()` appending to what it built instead of replacing it. The method is public and can be called again to refresh or swap a toolbar, and a second call left a duplicate of every button, label, separator, hotkey, and event handler rather than rebuilding
- Fixed `CompressionUtils.Decompress()` reporting a successful decompression for a format it never handled. Anything that wasn't `.zip` or `.gz` fell out of the extension check having already logged "Decompressing", so a caller couldn't tell it apart from an archive that extracted. It throws an `InvalidDataException` naming the extension now
- Fixed `ItemCollectionUI.InitializeContext()` binding to a context that can't marshal anywhere. It fell back to a new `SynchronizationContext` when there wasn't one to capture, which made `UsePost` true from then on, so every later add, insert, replace, and clear was posted to the thread pool — including ones made on the UI thread that would otherwise have run in place. The fallback is gone, and a null context runs them on the calling thread, which `UsePost` already returned false for. A later `InitializeContext()` can also pick up a real context now, where the unusable one used to be kept
- Fixed `BookmarkNavigator` reporting property changes off the thread that made them. Its constructor fell back to a new `SynchronizationContext` when there wasn't one to capture, and posting to a bare context queues to the thread pool, so a navigator built on a background thread sent every `PropertyChanged` to a pool thread — including the ones raised on the UI thread, which would otherwise have stayed there. The fallback is gone, and a null context invokes directly on the thread that changed the property, which `NotifyPropertyChanged()` already did
- Fixed `BookmarkNavigator` throwing on a history index outside the history. `Update()` indexed `History[CurrentIndex]` without the bounds check `Current` uses, `SeekBackward()` compared only against zero so an index past the end stepped back to another position still outside it, and `SeekForward()` covered only the upper bound so a negative index reached the history. `History` and `CurrentIndex` are both public settable and deserialized, so a restored navigator threw `ArgumentOutOfRangeException` during ordinary navigation. All three report that there's no bookmark there instead
- Fixed chart binning sizing its arrays from the data's own range. `LiveChartSeries.BinDataPoints()` divided the distance between the first and last X by the configured bin size and allocated a `double[]` and an `int[]` of that many entries, so two points far apart asked for far more bins than a chart has pixels: a billion of them claimed about 12 GB while still appearing to succeed, since the pages are only committed as they're written. Past that the count stopped fitting in an `int`, and the conversion saturates rather than wrapping, so the increment after it turned the size negative and threw `OverflowException`. `DateTime` values are binned in ticks, where a year is 3.15e14 of them, so both were reachable on ordinary data. The bin size widens to keep the count within `MaxBins` now, and a NaN or infinite X leaves the points unbinned rather than being divided into a meaningless number of them
- Fixed extracting an archive without any limit on how far it expands. `CompressionUtils.ExtractZip()` handed the archive straight to `ZipFile.ExtractToDirectory()`, so a small one could fill the disk it was extracted onto, and an archive of many tiny entries cost a file and its metadata each. `ExtractGzip()` copied its stream out the same unbounded way. A zip is checked against the sizes it declares before anything is written, a gzip is counted as it reads, and both stop at the new `CompressionUtils.MaxExtractedSize` (1 GB) with `MaxEntries` (100,000) capping the count
- Fixed an imported bookmark decompressing without any limit. `Bookmark.Create()` passes caller supplied base64 into `SerializerMemory.LoadBase64String()`, which copied the whole GZip stream into memory before any of it was validated, so a payload small enough to paste into a link could exhaust the process. The expansion stops at the new `SerializerMemory.MaxDecompressedSize` (100 MB) now, and `ValidateBase64()` uses the same path so it is bounded too
- Fixed restoring a window throwing on a display narrower than its minimum size. `BaseWindow.WindowSettings` clamped the saved width between `MinWidth` (700 by default) and the width `GetMaxBounds()` measured, and `Math.Clamp()` throws when the maximum is below the minimum. That happens on a narrow display, and whenever `Screens.All` is empty so the measured width is 0, which a remote session or a monitor change passes through. It threw while loading saved settings, before the window existed to report it. The size keeps its minimum now and the position follows, so the window opens larger than the space measured for it rather than not at all
- Fixed a closed tab leaving its tasks running. `TabInstance.Dispose()` cancelled what was in `Model.Tasks`, which is the collection the tab *displays*: `AddTask()` only adds a task that opted into being shown, and it caps at `MaxTasks` (10) by dropping the oldest. So a task started with `showTask: false` — the default for `StartAsync()`, `Post()`, and `ToolButton` — was never cancelled, and a shown one stopped being cancelled once the tab had started more than ten. Every task is tracked for cancellation now, separately from the ones displayed, and finished tasks are dropped as new ones are added rather than held for as long as the tab is open
- Fixed a browser save reporting success when nothing was stored. `SerializerLocalStorage.SaveInternal()` caught every serialization and interop failure, and treated localStorage rejecting the write as a warning, so `SerializerFile.Save()` and the repository above it returned normally with an index entry published for data that was never written, and callers could discard the only copy they had. A rejected write throws a `SerializerException` now, and serialization and interop failures reach the caller, matching what the file serializers already did
- Fixed `TimeRangePeriod.Periods()` aggregating a period's worth of data from after the requested time window. The window is extended by one period so the trailing boundary still gets one, but values were filtered and clipped against that extended end, so chart counts, sums, and averages included observations past the window while `TotalMinimum()` and `TotalMaximum()` on the same data excluded them. The window end is exclusive now, matching them
- Fixed a struct declaring a constructor that binds to none of its members being skipped entirely. Declaring one holds a struct back from the empty constructor so read only members can be restored through the declared one, and when nothing bound to it the type was left with no constructor at all, which made it unloadable and dropped it without reporting anything. `Activator` always creates a value type, so that's what it falls back to now
...
- Fixed `HttpMemoryCache` keying entries only by URI, which made requesting a cached URI as another type throw an `InvalidCastException`. Each response type now has its own entry for the URI, and `Add()` takes the type as a parameter rather than reading it off the object, so it agrees with what `TryGetValue<T>()` looks up. Storing through a base typed reference and asking for that base type would otherwise miss
- Fixed `HttpMemoryCache` accepting a zero or negative expiration duration, which failed later when the first entry was added, and added `IDisposable` so its underlying `MemoryCache` can be released deterministically
