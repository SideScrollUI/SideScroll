	...
	where TModel : class
{
	...
	public TreeDataGrid DataGrid;
	public TabControlSearch? SearchControl;

	public FlatTreeDataGridSource<TModel>? Source;

	public event EventHandler<TabSelectionChangedEventArgs>? OnSelectionChanged;

	private Dictionary<string, IColumn<TModel>> _columnObjects = [];
	private Dictionary<IColumn<TModel>, string> _columnNames = [];
	...
	public IList? Items
	{
		...
		set
		{
			...
			if (DataGrid != null)
			{
				Source = new FlatTreeDataGridSource<TModel>((IEnumerable<TModel>)List);
	...
	public TabControlTreeDataGrid(TabInstance tabInstance, IList iList, bool autoGenerateColumns, TabDataSettings? tabDataSettings = null, TabModel? model = null)
	{
	...
	private void DataGrid_ColumnReordered(object? sender, DataGridColumnEventArgs e)
	{
		...
		foreach (IColumn<TModel> column in DataGrid.Columns)
		{
	...
	private void RowSelection_SelectionChanged(object? sender, global::Avalonia.Controls.Selection.TreeSelectionModelSelectionChangedEventArgs e)
	{
	...
	private void SelectDefaultItems()
	{
		...
		if (firstValidObject != null && DataGrid.RowSelection?.SelectedItems.Count == 0)
	...
	{
		get => DataGrid.RowSelection!.SelectedItems.ToList();
		set
		{
			var dataGridSelectedItems = DataGrid.RowSelection!.SelectedItems;
	...
	{
		get => DataGrid.RowSelection.SelectedIndex[0];
		set => DataGrid.RowSelection.SelectedIndex = value;
	}

	public object? SelectedItem
	{
		get => DataGrid.RowSelection.SelectedItem;
		set
		{
			//autoSelectItem = null;
			if (value == null)
	...
	public HashSet<SelectedRow> SelectedRows
	{
		get
		{
			...
			try
			{
				foreach (object obj in DataGrid.RowSelection.SelectedItems)
				{
					if (obj == null)
						continue;
